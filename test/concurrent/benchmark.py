...
def sample_requests(
    ...
    random.seed(seed)
    np.random.seed(seed)
    tokenizer = LlamaTokenizer.from_pretrained(model_path, trust_remote_code=True, legacy=False)
    print(dataset_path)
...
MODEL = "/mnt/disk1/models/Llama-2-13b-chat-hf"
PROMPT = PROMPT_1024
MAX_TOKENS = 128


for MAX_CONCURRENT_REQUESTS in [1]:
    NUM_WARMUP = 10 * MAX_CONCURRENT_REQUESTS
    NUM_REQUESTS = 100 * MAX_CONCURRENT_REQUESTS  # 总请求次数
